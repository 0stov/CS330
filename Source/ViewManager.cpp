...
/*
This function processes the mouse wheel movement.
It will set the varaible that stores our speed moving the camera around.
*/
void ViewManager::scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	//most mouses only hae a y axis to adjust. so - or + we add it to our current multiplier
	move_mult += yoffset;

	//the following if statements correct for a max and min movement multiplier
	//we don't want to move too fast or we'll lose the schene (trust me)
	if (move_mult > 2) {
		move_mult == 50.0;
	}
	//if movement goes below zero our control will be inverted. we don't want that.
	else if (move_mult < 1.0) {
		move_mult == 1.0;
	}
}
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	{
		g_pCamera->ProcessKeyboard(FORWARD, gDeltaTime * move_mult);
	}
	if (glfwGetKey(m_pWindow, GLFW_KEY_S) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(BACKWARD, gDeltaTime * move_mult);
	}

	// process camera panning left and right
	if (glfwGetKey(m_pWindow, GLFW_KEY_A) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(LEFT, gDeltaTime * move_mult);
	}
	if (glfwGetKey(m_pWindow, GLFW_KEY_D) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(RIGHT, gDeltaTime * move_mult);
	}

	// process camera panning up and down
	if (glfwGetKey(m_pWindow, GLFW_KEY_Q) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(UP, gDeltaTime * move_mult);
	}
	if (glfwGetKey(m_pWindow, GLFW_KEY_E) == GLFW_PRESS)
	{
		g_pCamera->ProcessKeyboard(DOWN, gDeltaTime * move_mult);
	}
	/*
